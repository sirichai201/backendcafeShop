...
func CreateBillByUserID(c *fiber.Ctx) error {
	// สร้างบิลที่ลูกค้าสั่งสินค้าจาก order โดยค้นหา order จาก user_id และ status
	bill := new(models.Bill)
	id := c.Params("id")
	objID, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		log.Println("Invalid ID:", err)
		return c.Status(fiber.StatusBadRequest).SendString("รหัสไอดีไม่ถูกต้อง")
	}
	var orders []models.Orders
	filter := bson.M{
		"user_id": objID,
	...
	for _, order := range orders {
		...
		log.Println("ราคา :", product.ProductPrice)
		log.Println("แต้ม :", product.ProductPoint)
	...
	bill = &models.Bill{
		...
		Payment:     "0", // สมมติว่ามีฟังก์ชันคำนวณแต้ม
	}

	// บันทึกบิลในฐานข้อมูล
	result, err := database.BillsCollection.InsertOne(context.Background(), bill) // สมมติว่า BillsCollection เป็นคอลเลกชันที่ถูกต้อง
	if err != nil {
		log.Fatal(err)
	}
	log.Println("ผลลัพธ์ :", result)

	createdBill := new(models.Bill)
	err = database.BillsCollection.FindOne(context.Background(), bson.M{"_id": result.InsertedID}).Decode(&createdBill)
	if err != nil {
		log.Fatal(err)
	}
	...
	response := struct {
		...
		Bill_id string       `json:"id"` // เพิ่มฟิลด์ ID ของบิล

	}{
		Message: "บิลถูกสร้างเรียบร้อยแล้ว",
		Bill:    createdBill,
		// เพิ่มฟิลด์ ID ของบิล
		// กำหนดค่าเป็น ID ของบิลที่ถูกสร้าง
	}
